...
- For a simulator package, run `tooling/scripts/package_ios_release.sh`; it leaves `dist/InfoMatrix-iOS.app` in place and zips the same bundle to `dist/releases/ios/InfoMatrix-iOS-simulator.zip`. That artifact is for local smoke testing and is not published as a public GitHub Release download.
